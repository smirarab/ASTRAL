...
* Download using one of two approaches:
    * You simply need to download the [zip file](https://github.com/smirarab/ASTRAL/raw/master/Astral.5.7.6.zip) and extract the contents to a folder of your choice. 
...
* To test your installation, go to the place where you put the uncompressed ASTRAL, and run:
  ...
  ``` bash
   java -jar astral.5.7.6.jar -i test_data/song_primates.424.gene.tre
   ```
...
* ASTRAL can be run from any directory (e.g., `/path/to/astral/`). Then, you just need to run:
  ...
  ``` bash
  java -jar /path/to/astral/astral.5.7.6.jar
  ```

* Also, you can move `astral.5.7.6.jar` to any location you like and run it from there, but note that you need to move the `lib` directory with it as well.
...
```
java -jar astral.5.7.6.jar -i in.tree
```

The results will be outputted to the standard output. To save the results in a file use the `-o` option (**Strongly recommended**):

```
java -jar astral.5.7.6.jar -i in.tree -o out.tre
```
To save the logs (**also recommended**), run:

```
java -jar astral.5.7.6.jar -i in.tree -o out.tre 2>out.log
```
...
```
java -Xmx8000M -jar astral.5.7.6.jar -i in.tree
```
