...
There is no installation required to run ASTRAL. You simply need to download the [zip file](https://github.com/smirarab/ASTRAL/raw/master/Astral.4.7.9.zip) and extract the contents to a folder of your choice. Alternatively, you can clone the [github repository](https://github.com/smirarab/ASTRAL/) if you are familiar with git. If you clone the git repository, you can run `make.sh` to build the project, or simply use the jar file that is included with the repository. 
...
```
java -jar astral.4.7.9.jar -i test_data/song_mammals.424.gene.tre
```

The results will be outputted to the standard output. To save the results in an output file use the `-o` option:

```
java -jar astral.4.7.9.jar -i test_data/song_mammals.424.gene.tre -o test_data/song_mammals.tre
```
...
**However,** it should be emphasized that the current handling of multiple taxa is not well tested and needs more work. 
Please stay tuned for upcoming improvements that fix/improve this feature. 
...
```
java -jar astral.4.7.9.jar -i test_data/song_primates.424.gene.tre -o test_data/song_primates.424.exact.tre -x
```
...
```
java -jar astral.4.7.9.jar -i test_data/song_primates.424.gene.tre -o test_data/song_primates.424.default.tre
```
...
```
java -jar astral.4.7.9.jar -i test_data/simulated_14taxon.gene.tre -o test_data/simulated_14taxon.default.tre
```

and then

```
java -jar astral.4.7.9.jar -i test_data/simulated_14taxon.gene.tre -o test_data/simulated_14taxon.exact.tre -x
```
...
```
java -jar astral.4.7.9.jar -i test_data/simulated_primates_5X.10.gene.tre -o test_data/simulated_primates_5X.10.species.tre -e test_data/simulated_primates_5X.10.bootstrap.gene.tre
```
...
```
java -jar astral.4.7.9.jar -i test_data/100-simulated-boot
```
...
```
java -jar ../astral.4.7.9.jar -i song_mammals.424.gene.tre -b bs-files
```
...
```
java -jar ../astral.4.7.9.jar -i song_mammals.424.gene.tre -b bs-files -r 150
```
...
```
java -jar ../astral.4.7.9.jar -i song_mammals.424.gene.tre -b bs-files -g -r 100
```
...
```
java -jar astral.4.7.9.jar -q test_data/simulated_14taxon.default.tre -i test_data/simulated_14taxon.gene.tre
```
...
```
java -Xmx3000M -jar astral.4.7.9.jar -i in.tree
```
