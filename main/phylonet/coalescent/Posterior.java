...
public class Posterior {
	...
	public double post(){
		double g = G(m1,n)/(G(m1,n)+Functions.pow.apply(2.,m2-m1)*G(m2,n)*betaRatio(m2+1,n-m2+1,m1+1,n-m1+1)+Functions.pow.apply(2.,m3-m1)*G(m3,n)*betaRatio(m3+1,n-m3+1,m1+1,n-m1+1));
		if (Double.isNaN(g)){
			if (m1/n>1./3.) g=1.;
			else g=0;
		}
