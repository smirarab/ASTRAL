...
public class PolytreeA3 {
	...
	final class PTNode{
			...
			if (isUsed){
				...
				if (isPartitionNode()){
					...
					if (partition.cnt > 1){
						...
						PolytreeA3.this.queueBuilder.add(partition.cnt);
					}
		...
		boolean addToList(){
			return isUsed && isClusterNode() && cluster.listUsed;
		}
	}
	...
	final class PTCluster{
		...
		PTCluster(STITreeCluster c, PTNode n){
			clusterRef = c;
			firstNode = n;
			PolytreeA3.this.clusters.put(c, this);
		}
		PTCluster(STITreeCluster c){
			clusterRef = c;
			firstNode = null;
			listUsed = true;
			listPos = PolytreeA3.this.listSize++;
			PolytreeA3.this.clusters.put(c, this);
		}
	}
	...
	final class PTPartition{
		...
		PTPartition(AbstractPartition p, PTNode n){
			firstNode = n;
			PolytreeA3.this.partitions.put(p, this);
		}
	}
	...
	public Long WQWeightByTraversal(Tripartition trip, CondensedTraversalWeightCalculator algorithm){
		...
		setTime(getTime() + System.nanoTime() - t);
		return weight;
	}
