...
public class Polytree {
	...
	public Polytree(List<Tree> trees){
		...
		for (HashMap.Entry<AbstractPartition, Integer> entry : partitionCount.entrySet()){
		...
		dependerID = aDependerID.stream().mapToInt(i -> i).toArray();
		dependeeID = aDependeeID.stream().mapToInt(i -> i).toArray();
		dependingFactor = aDependingFactor.stream().mapToInt(i -> i).toArray();
		aDependerID = null;
		aDependeeID = null;
		aDependingFactor = null;
		partitionMultiplicity = aPartitionMultiplicity.stream().mapToInt(i -> i).toArray();
		partitionNumClusters = aPartitionNumClusters.stream().mapToInt(i -> i).toArray();
		partitionClusterID = aPartitionClusterID.stream().mapToInt(i -> i).toArray();
	...
	private STITreeCluster addSubtreeClusters(TNode node, STITreeCluster s){
		...
		if (s == null){
			...
			if (node.isRoot() == false){
				STITreeCluster xc = new STITreeCluster();
