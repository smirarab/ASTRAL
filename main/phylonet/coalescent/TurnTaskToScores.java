...
public class TurnTaskToScores implements Runnable {
	...
	public void run() {
		...
		if(CommandLine.timerOn) {
	       	System.err.println("TIME TOOK FROM LAST NOTICE TurnTaskToScores:92: " + (double)(System.nanoTime()-CommandLine.timer)/1000000000);
		...
		while(true) {
			...
			if(noGPU) {
				...
				for(int i = speciesWordLength - 1; i >= 0; i--)
					gpu.tripartitions1[currentGPU][(speciesWordLength - i - 1) * (int)workGroupSize + tripCounter]=task.cluster1.getBitSet().words[i];
				for(int i = speciesWordLength - 1; i >= 0; i--)
					gpu.tripartitions2[currentGPU][(speciesWordLength - i - 1) * (int)workGroupSize + tripCounter]=task.cluster2.getBitSet().words[i];
				for(int i = speciesWordLength - 1; i >= 0; i--)
...
//		System.out.println("Time used to wait was: " + timeWait/1000000000);
		...
		try {
			queue2Helper.offer(new ComparablePair(-23L, positionIn++)); //random specific number used as a "poison pill" for AbstractWeightCalculator
...
//		}
		...
		if(CommandLine.timerOn) {
			System.err.println("Time used to wait on queue1.take() with at least one gpu available: " + (double)(timeWait)/1000000000);
	       	System.err.println("TIME TOOK FROM LAST NOTICE TurnTaskToScores:199: " + (double)(System.nanoTime()-CommandLine.timer)/1000000000);
			CommandLine.timer = System.nanoTime();
		}
	}
	...
	public class GPUCall {
		...
		public void initCL() {
			...
			for(int i = 0; i < devices.length; i++) {
				...
				d_tripartitions1[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, Sizeof.cl_long * tripartitions1[i].length,
						null, null);
				d_tripartitions2[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, Sizeof.cl_long * tripartitions2[i].length,
						null, null);
				d_tripartitions3[i] = clCreateBuffer(context,  CL_MEM_READ_ONLY, Sizeof.cl_long * tripartitions3[i].length,
						null, null);
		...
		public class ComputeThread implements Runnable{
			...
			public void run() {
				...
				do {
					...
					clEnqueueWriteBuffer(commandQueues[deviceIndex], d_tripartitions1[deviceIndex], CL_TRUE, 0L, Sizeof.cl_long * tripartitions1[deviceIndex].length, Pointer.to(tripartitions1[deviceIndex]), 0,
							null, null);
					clEnqueueWriteBuffer(commandQueues[deviceIndex], d_tripartitions2[deviceIndex], CL_TRUE, 0L, Sizeof.cl_long * tripartitions2[deviceIndex].length, Pointer.to(tripartitions2[deviceIndex]), 0,
							null, null);
					...
					synchronized(kernel) {
						...
						clEnqueueNDRangeKernel(commandQueues[deviceIndex], kernel, 1, null, new long[]{workSize}, null, 0, null, null);
					}
					clEnqueueReadBuffer(commandQueues[deviceIndex], d_weightArray[deviceIndex], CL_TRUE, 0L, Sizeof.cl_long * weightArray[deviceIndex].length, Pointer.to(weightArray[deviceIndex]), 0, null, null);
					for(int i = 0; i < workSize; i++) {
						queue2Helper.offer(new ComparablePair(weightArray[deviceIndex][i], label[labelIndex][deviceIndex][i]));
//						System.out.println("I have been here!" + queue2Helper.peek().value + " " + positionOut);
								...
								if (overlapind[i][0] != overlapind[j][1])
									for (int k = nzc[2] - 1; k >= 0; k--) {
										if ((overlapind[i][0] != overlapind[k][2]) &&
											...
											weights[ii] += F(overlap[i][0], overlap[j][1], overlap[k][2]);
									}
							}
						}
						top = top + gtb + 1;
